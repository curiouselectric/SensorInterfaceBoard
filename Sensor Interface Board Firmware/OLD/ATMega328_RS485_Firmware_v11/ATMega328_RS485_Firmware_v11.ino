...
float getSensorData(int _channel_number) {
  ...
  return (soilMoistureSensor.soilTemperatureC);
}
...
#include "soil_moisture_rs485.h"
  my_sensor_data[0].data_send_time = 1;  // 1 second reads for moisture
  my_sensor_data[1].data_send_time = 1;  // 1 second reads for temperature
  ...
  t3600S.enable();
#ifdef DEBUG_FLAG
  Serial.println(F("Enabled Tasks"));
#endif
